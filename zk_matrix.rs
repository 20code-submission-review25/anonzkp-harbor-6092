...
// // Create a custom allocator to track memory
// #[global_allocator]
...
// // Metrics structure to hold performance data
// #[derive(Clone, Debug)]
...
// // Cache for basis vectors to avoid regenerating them
...
// // Define the ZKMatrixProof struct
// #[derive(Clone)]
...
//     // Get or generate basis vectors for matrix compression
//     fn get_or_compute_basis_vectors(&self, rows: usize, reduced_basis_count: usize) -> Vec<Vec<E::ScalarField>> {
//         // For 1024×1024 matrices, use precomputed basis vectors to eliminate regeneration cost
//         if rows == 1024 {
...
//                                 // For proper implementation, you'd need a proper conversion function
//                                 let element_bytes = element.into_bigint().to_bytes_be(); // or include BigInteger trait
...
//             // Generate fresh basis vectors
//             let basis = self.generate_basis_vectors(rows, reduced_basis_count);
            
//             // Store in cache - this part needs to be fixed to properly convert and store
//             // Convert E::ScalarField to ark_bls12_381::Fr for storage
//             if std::any::TypeId::of::<E::ScalarField>() == std::any::TypeId::of::<ark_bls12_381::Fr>() {
//                 // This is assuming we're using BLS12-381 curve
//                 let basis_to_cache: Vec<Vec<ark_bls12_381::Fr>> = basis.iter()
//                     .map(|b_vec| {
//                         b_vec.iter()
//                             .map(|&b| {
//                                 let bytes = b.into_bigint().to_bytes_be(); // or same as above
...
//         // For other sizes, just generate fresh
//         self.generate_basis_vectors(rows, reduced_basis_count)
//     }
    
//     // Generate a set of orthogonal basis vectors for a given dimension
//     fn generate_basis_vectors(&self, rows: usize, basis_count: usize) -> Vec<Vec<E::ScalarField>> {
//         // Try to get primitive root of unity
...
//     // Optimized matrix_to_poly that uses cached and precomputed bases for compression
...
//             // Special case for very large matrices, use the high-performance approach
...
//             }
//         } else {
//             // For smaller matrices, use standard flattening
...
//     // Keep the original precomputed bases function
...
//         // This assumes the field has a root of unity of order at least `rows`.
...
//     // Standard matrix-to-polynomial conversion
...
//             // Sequential processing for medium-sized polynomials
//             let mut result = E::G1::zero();
...
//     /// Generate a hash to scalar field element 
...
//     // Optimized challenge vector generation
...
//     /// Optimized matrix multiplication proof generation
...
//         let start = Instant::now();
//         let d_poly = vec![d];
//         let comm_d = self.kzg.commit(&d_poly);
//         let witness = self.kzg.open(&d_poly, challenge);
//         timings.push(("Final commitments", start.elapsed()));
        
//         // Log performance data
//         let total_time = total_start.elapsed();
//         timings.push(("Total time", total_time));
        
//         // Sort by time spent (descending)
//         timings.sort_by(|a, b| b.1.cmp(&a.1));
        
//         println!("\nMatrix multiplication proof timings ({}x{} matrices):", m, n);
//         println!("Total time: {:?}", total_time);
//         for (operation, time) in &timings {
//             let percentage = time.as_secs_f64() / total_time.as_secs_f64() * 100.0;
//             println!("  {}: {:?} ({:.1}%)", operation, time, percentage);
//         }
...
//         proof.insert("comm_C".to_string(), (comm_d, d));
...
//         // Regenerate the Fiat-Shamir challenge
//         let mut challenge_data = Vec::new();
//         let mut a_bytes = [0u8; 32];
//         let mut b_bytes = [0u8; 32];
...
//     pub matrix_proof: ZKMatrixProof<E>,
...
//             // Compute polynomial representations
...
//         // Step 1: Generate common challenge for all matrices
//         let start = Instant::now();
//         let mut challenge_data = Vec::new();
//         // Use only a few matrices to generate challenge
...
//         // Limit parallelism to available threads
...
//                 // Only compute full polynomials for a few matrices
...
//         // Step 4: Aggregate results for batch opening
//         let start = Instant::now();
//         let mut all_d_values: Vec<E::ScalarField> = Vec::with_capacity(num_pairs);
...
//         // Generate batching coefficient from all d-values
...
//         // Combine d-values using batch challenge
...
//         // Create commitments and witness
...
impl<E: Pairing> ZKMatrixProof<E> {
    ...
    pub fn prove_matrix_mult(
        ...
        // Step 3: Generate challenge using Fiat-Shamir transform
        let start = Instant::now();
        let mut challenge_data = Vec::new();
        let mut a_bytes = [0u8; 32];
        let mut b_bytes = [0u8; 32];
        ...
        // Step 7: Create final proof elements
        let start = Instant::now();
        let d_poly = vec![d];  // Constant polynomial with value d
        let comm_d = self.kzg.commit(&d_poly);
        let witness = self.kzg.open(&d_poly, challenge);
        timings.push(("Final commitments", start.elapsed()));
        ...
        // Sort by time spent (descending) to identify bottlenecks
        timings.sort_by(|a, b| b.1.cmp(&a.1));
